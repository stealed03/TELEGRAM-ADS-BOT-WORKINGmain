...
async def process_forward_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = update.message
    if not msg.forward_date and not msg.forward_from and not msg.forward_from_chat and not msg.forward_sender_name:
        await msg.reply_text(
            "❌ Please *forward* a message (don't type — use Telegram's forward button).",
            parse_mode='Markdown'
        )
        return True

    # Get origin chat id and message id
    # Telethon needs the original source chat + message id to forward
    # With PTB we store: the chat we received the forward IN + the message_id of that forwarded copy
    # Then Telethon will forward FROM this bot's chat using that message

    context.user_data['forward_source_chat_id'] = msg.chat_id
    context.user_data['forward_message_id'] = msg.message_id
    context.user_data['awaiting_forward_msg'] = False
    context.user_data['awaiting_forward_interval'] = True

    await msg.reply_text(
        "✅ Message saved!\n\nNow send the *interval in minutes* (how often to forward):\n\nExample: `30`",
        parse_mode='Markdown'
    )
    return True
...
async def process_forward_interval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await db.set_forward_config(user_id, account['id'], source_chat_id, message_id, interval)
