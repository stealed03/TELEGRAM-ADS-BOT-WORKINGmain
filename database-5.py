...
class Database:
    ...
    async def init_db(self):
        async with aiosqlite.connect(self.db_name) as db:
            ...
            # All-groups broadcast config (one row per user)
            await db.execute('''
            ...
            # Broadcast send history — one row per group per broadcast round
            await db.execute('''
            ...
            # Forward message config — stores which message to forward to all groups
            await db.execute('''
                CREATE TABLE IF NOT EXISTS forward_config (
                    ...
                    message_id INTEGER,
                    forward_text TEXT,
                    is_original_source BOOLEAN DEFAULT 0,
                    interval_minutes INTEGER DEFAULT 60,
                    is_active BOOLEAN DEFAULT 0,
                    last_sent TIMESTAMP,
                    FOREIGN KEY (user_id) REFERENCES users(user_id)
    ...
    async def get_broadcast_config(self, user_id):
        async with aiosqlite.connect(self.db_name) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                'SELECT * FROM broadcast_config WHERE user_id = ?', (user_id,)
            ) as cursor:
                row = await cursor.fetchone()
                return dict(row) if row else None

    async def set_broadcast_config(self, user_id, account_id, message, interval_minutes):
        async with aiosqlite.connect(self.db_name) as db:
            await db.execute(
                '''INSERT INTO broadcast_config (user_id, account_id, message, interval_minutes, is_active)
                   VALUES (?, ?, ?, ?, 0)
                   ON CONFLICT(user_id) DO UPDATE SET
                       account_id=excluded.account_id,
                       message=excluded.message,
                       interval_minutes=excluded.interval_minutes''',
                (user_id, account_id, message, interval_minutes)
            )
            await db.commit()

    async def toggle_broadcast(self, user_id):
        """Flip is_active, return new state (True/False)."""
        async with aiosqlite.connect(self.db_name) as db:
            async with db.execute(
                'SELECT is_active FROM broadcast_config WHERE user_id = ?', (user_id,)
            ) as cursor:
                row = await cursor.fetchone()
            if row is None:
                return False
            ...
            await db.execute(
                'UPDATE broadcast_config SET is_active = ? WHERE user_id = ?',
                (new_state, user_id)
            )
            await db.commit()
            return bool(new_state)

    async def get_all_active_broadcasts(self):
        async with aiosqlite.connect(self.db_name) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                'SELECT * FROM broadcast_config WHERE is_active = 1 AND message IS NOT NULL'
            ) as cursor:
                rows = await cursor.fetchall()
                return [dict(row) for row in rows]

    async def update_broadcast_last_sent(self, user_id):
        async with aiosqlite.connect(self.db_name) as db:
            await db.execute(
                "UPDATE broadcast_config SET last_sent = datetime('now') WHERE user_id = ?",
                (user_id,)
            )
            await db.commit()

    async def log_broadcast(self, user_id, group_id, group_title, status='sent', error=None):
        """Log one group's broadcast result."""
        async with aiosqlite.connect(self.db_name) as db:
            await db.execute(
                '''INSERT INTO broadcast_log (user_id, group_id, group_title, status, error)
                   VALUES (?, ?, ?, ?, ?)''',
                (user_id, group_id, group_title, status, error)
            )
            await db.commit()

    async def get_broadcast_history(self, user_id, limit=100):
        """Get last N broadcast log entries for a user."""
        async with aiosqlite.connect(self.db_name) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                '''SELECT * FROM broadcast_log WHERE user_id = ?
    ...
    async def get_broadcast_stats(self, user_id):
        """Get total sent/failed counts from last broadcast round."""
        async with aiosqlite.connect(self.db_name) as db:
            db.row_factory = aiosqlite.Row
            # last_sent time from config
            async with db.execute(
                'SELECT last_sent FROM broadcast_config WHERE user_id = ?', (user_id,)
            ) as cur:
                cfg = await cur.fetchone()
            if not cfg or not cfg['last_sent']:
                return {'sent': 0, 'failed': 0, 'total': 0, 'last_sent': None}
            last_sent = cfg['last_sent']
            async with db.execute(
                '''SELECT status, COUNT(*) as cnt FROM broadcast_log
                   WHERE user_id = ? AND sent_at >= ?
                   GROUP BY status''',
                (user_id, last_sent)
            ) as cur:
                rows = await cur.fetchall()
            stats = {'sent': 0, 'failed': 0, 'last_sent': last_sent}
            for r in rows:
    ...
    async def get_forward_config(self, user_id):
        async with aiosqlite.connect(self.db_name) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                'SELECT * FROM forward_config WHERE user_id = ?', (user_id,)
            ) as cursor:
                row = await cursor.fetchone()
                return dict(row) if row else None

    async def set_forward_config(self, user_id, account_id, source_chat_id, message_id, interval_minutes, forward_text="", is_original_source=False):
        async with aiosqlite.connect(self.db_name) as db:
            await db.execute(
                '''INSERT INTO forward_config (user_id, account_id, source_chat_id, message_id, forward_text, is_original_source, interval_minutes, is_active)
                   VALUES (?, ?, ?, ?, ?, ?, ?, 0)
                   ON CONFLICT(user_id) DO UPDATE SET
                       account_id=excluded.account_id,
                       source_chat_id=excluded.source_chat_id,
                       message_id=excluded.message_id,
                       forward_text=excluded.forward_text,
                       is_original_source=excluded.is_original_source,
                       interval_minutes=excluded.interval_minutes''',
                (user_id, account_id, source_chat_id, message_id, forward_text, int(is_original_source), interval_minutes)
            )
            await db.commit()

    async def toggle_forward(self, user_id):
        """Flip is_active for forward config, return new state."""
        async with aiosqlite.connect(self.db_name) as db:
            async with db.execute(
                'SELECT is_active FROM forward_config WHERE user_id = ?', (user_id,)
            ) as cursor:
                row = await cursor.fetchone()
            if row is None:
                return False
            new_state = 0 if row[0] else 1
            await db.execute(
                'UPDATE forward_config SET is_active = ? WHERE user_id = ?',
                (new_state, user_id)
            )
            await db.commit()
            return bool(new_state)

    async def get_all_active_forwards(self):
        async with aiosqlite.connect(self.db_name) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                'SELECT * FROM forward_config WHERE is_active = 1 AND message_id IS NOT NULL'
            ) as cursor:
                rows = await cursor.fetchall()
                return [dict(row) for row in rows]

    async def update_forward_last_sent(self, user_id):
        async with aiosqlite.connect(self.db_name) as db:
            await db.execute(
                "UPDATE forward_config SET last_sent = datetime('now') WHERE user_id = ?",
                (user_id,)
            )
            await db.commit()
