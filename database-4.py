...
class Database:
    ...
    async def set_forward_config(self, user_id, account_id, source_chat_id, message_id, interval_minutes):
        async with aiosqlite.connect(self.db_name) as db:
            await db.execute(
                '''INSERT INTO forward_config (user_id, account_id, source_chat_id, message_id, interval_minutes, is_active)
                   VALUES (?, ?, ?, ?, ?, 0)
                   ON CONFLICT(user_id) DO UPDATE SET
                       account_id=excluded.account_id,
                       ...
                       interval_minutes=excluded.interval_minutes''',
                (user_id, account_id, source_chat_id, message_id, interval_minutes)
            )
            await db.commit()
