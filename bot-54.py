...
async def auto_start_users():
    ...
    Also notifies users whose session was revoked by Telegram."""
                    ...
                    if account:
                        try:
                            ...
                            if client is None:
                                ...
                                print(f"⚠️ Session expired for user {user_id}, notifying...")
                                try:
                                    await bot.send_message(
                                        ...
                                        text=(
                                            ...
                                            "Telegram ne aapki session revoke kar di hai "
                                            "(ye normal hai — Telegram kabhi kabhi sessions hatata hai).\n\n"
                                            "🔐 *Dobara login karo:*\n"
                                            "➡️ /start → Login button dabao\n\n"
                                            "API ID aur API Hash wahi use karo jo pehle use kiye the."
                                        ),
                                        parse_mode='Markdown'
                                    )
                                except Exception:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Your automation bot is ready\\!\n\n"
        f"──────────────────────\n"
        f"      ⚡ *FEATURES*\n"
        f"──────────────────────\n"
        f"  💬  Instant Messaging\n"
        ...
        f"    @Drowned\\_Whale\n\n"
        f"🧑‍💻 Developer\n"
        f"📩 Contact for Paid Bots in affordable rates\n"
        f"@Talk\\_with\\_stealed\n\n"
        f"🌟━━━━━━━━━━━━━━━━━━━━🌟",
        parse_mode='MarkdownV2',
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("👑 Owner", url="https://t.me/Drowned_Whale"),
            InlineKeyboardButton("🧑‍💻 Developer", url="https://t.me/Talk_with_stealed")
        ]])
    )
...
async def group_messages_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    fwd_config = await db.get_forward_config(user_id)
    forward_active = bool(fwd_config and fwd_config.get('is_active'))
    ...
    fwd_info = ""
    if fwd_config and fwd_config.get('message_id'):
        fwd_info = f"\n\n📨 *Forward:* msg_id `{fwd_config['message_id']}` every `{fwd_config['interval_minutes']}` min"

    text = "\U0001f4e2 *All Groups Broadcast*" + info + fwd_info + "\n\nSends your message to *all joined groups* at the set interval."
    try:
        await query.edit_message_text(
            text,
            reply_markup=menu_ui.group_messages_menu(is_active=is_active, forward_active=forward_active),
            parse_mode='Markdown'
        )
    except BadRequest:
        pass
...
async def gm_forward_set_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Prompt user to forward a message and set interval."""
    query = update.callback_query
    await query.answer()
    user_id = update.effective_user.id
    ...
    if fwd_config and fwd_config.get('message_id'):
        current_info = (
            f"\n\n📌 *Current:* msg_id `{fwd_config['message_id']}` "
            f"every `{fwd_config['interval_minutes']}` min "
            f"({'🟢 ON' if fwd_config.get('is_active') else '🔴 OFF'})"
        )

    try:
        await query.edit_message_text(
            f"📨 *Set Forward Message*{current_info}\n\n"
            "**Step 1:** Forward any message to this bot right now "
            "(from a group, channel, or your saved messages).\n\n"
            "**Step 2:** Then reply with just the interval in minutes.\n\n"
            "Example: forward a message → bot asks interval → you type `30`\n\n"
            "⚡ Bot will forward that exact message (with emojis/media/stickers) "
            "to *all your joined groups* every N minutes.",
            parse_mode='Markdown',
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("« Cancel", callback_data="group_messages")
            ]])
        ...
        pass
    context.user_data['awaiting_forward_msg'] = True


async def process_forward_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive the forwarded message from user and ask for interval."""
    if not context.user_data.get('awaiting_forward_msg'):
        return False  # not our turn

    user_id = update.effective_user.id
    msg = update.message

    # Check if it's a forwarded message (any forward indicator)
    is_forwarded = bool(
        msg.forward_date or
        msg.forward_from or
        msg.forward_from_chat or
        msg.forward_sender_name or
        msg.forward_origin  # newer PTB versions
    )

    if not is_forwarded:
        await msg.reply_text(
            "❌ Ye forwarded message nahi hai!\n\n"
            "Telegram mein kisi message pe *long press* karo → *Forward* dabao → is bot ko bhejo.",
            parse_mode='Markdown'
        )
        return True

    # Store the forwarded copy info sitting in THIS bot chat.
    # We also try to get the original source for Telethon to forward from.
    source_chat_id = None
    source_msg_id = None

    # Try to get original channel/group source
    if msg.forward_from_chat:
        source_chat_id = msg.forward_from_chat.id
        source_msg_id = msg.forward_from_message_id

    # Fallback: store the copy in bot's PM — Telethon will copy text/caption instead
    if not source_chat_id:
        source_chat_id = msg.chat_id  # bot's PM with user
        source_msg_id = msg.message_id

    # Also save text/caption for fallback copy-send
    context.user_data['forward_source_chat_id'] = source_chat_id
    context.user_data['forward_message_id'] = source_msg_id
    context.user_data['forward_text'] = msg.text or msg.caption or ""
    context.user_data['forward_is_original_source'] = bool(msg.forward_from_chat)
    context.user_data['awaiting_forward_msg'] = False
    context.user_data['awaiting_forward_interval'] = True

    preview = (msg.text or msg.caption or "[media/sticker]")[:60]
    await msg.reply_text(
        f"✅ *Message capture ho gaya!*\n\n"
        f"📝 Preview: `{preview}`\n\n"
        f"Ab interval bhejo — kitne minutes mein forward karna hai?\n\n"
        f"Example: `30`",
        parse_mode='Markdown'
    )
    return True
...
async def process_forward_interval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive interval and save forward config."""
    if not context.user_data.get('awaiting_forward_interval'):
        return False
    ...
    try:
        ...
        await update.message.reply_text("❌ Send a number (minutes), e.g. `30`", parse_mode='Markdown')
        return True

    source_chat_id = context.user_data.get('forward_source_chat_id')
    message_id = context.user_data.get('forward_message_id')

    if not source_chat_id or not message_id:
        await update.message.reply_text("❌ Something went wrong. Please start again from Group Messages menu.")
        context.user_data['awaiting_forward_interval'] = False
        return True
    ...
    await db.set_forward_config(
        user_id, account['id'],
        source_chat_id, message_id, interval,
        forward_text=context.user_data.get('forward_text', ''),
        is_original_source=context.user_data.get('forward_is_original_source', False)
    )

    # Auto-enable
    fwd_config = await db.get_forward_config(user_id)
    if not fwd_config.get('is_active'):
        await db.toggle_forward(user_id)

    context.user_data['awaiting_forward_interval'] = False

    await update.message.reply_text(
        f"✅ *Forward configured & started!*\n\n"
        f"⏱ Every *{interval}* min\n"
        f"📨 Will forward that message to *all joined groups*\n\n"
        f"🟢 Running automatically. Go to Group Messages → Stop Forward to stop.",
        parse_mode='Markdown',
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("📢 Group Messages", callback_data="group_messages")
        ]])
...
async def gm_forward_toggle_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    fwd_config = await db.get_forward_config(user_id)
    if not fwd_config or not fwd_config.get('message_id'):
        try:
            await query.answer("⚠️ Set a forward message first!", show_alert=True)
        except Exception:
            pass
        return

    new_state = await db.toggle_forward(user_id)
    status = "✅ ON — forwarding started!" if new_state else "❌ OFF — forwarding stopped."

    config = await db.get_broadcast_config(user_id)
    is_active = bool(config and config.get('is_active'))
    fwd_config = await db.get_forward_config(user_id)

    text = f"📨 *Forward Message*\n\n{status}"
    try:
        await query.edit_message_text(
            text,
            reply_markup=menu_ui.group_messages_menu(is_active=is_active, forward_active=new_state),
            parse_mode='Markdown'
        )
    except BadRequest:
        pass
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif context.user_data.get('awaiting_forward_msg'):
        # Can be forwarded message or text — process_forward_message handles it
        await process_forward_message(update, context)
    elif context.user_data.get('awaiting_forward_interval'):
        await process_forward_interval(update, context)
    else:
        await update.message.reply_text(
            "👋 Use /start for menu!",
            reply_markup=menu_ui.main_menu(is_admin=is_admin(user_id), broadcast_active=False)
...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Catch ALL messages (including forwarded ones) for the forward setup flow."""
    user_id = update.effective_user.id
    if context.user_data.get('awaiting_forward_msg'):
        await process_forward_message(update, context)
...
async def post_init(application: Application):
    ...
    asyncio.create_task(forward_message_manager.start_forward_job())
    print("✅ Bot initialized!")
    print("⏰ Scheduler running")
    print("📢 Group message job started")
    print("📨 Forward message job started")
    print("📡 Auto-start complete")

# ============ MAIN ============

async def login_in_progress_warning(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Tell user login is in progress when they type any command mid-login."""
    await update.message.reply_text(
        "⚠️ *Login chal raha hai!*\n\n"
        "Pehle login complete karo.\n"
        "Beech mein commands kaam nahi karenge.\n\n"
        "➡️ Jo step pe ho woh complete karo, ya /cancel likho login band karne ke liye.",
        parse_mode='Markdown'
    )
...
def main():
    ...
    login_conv = ConversationHandler(
        ...
        fallbacks=[
            ...
            CommandHandler('start', login_in_progress_warning),
            MessageHandler(filters.COMMAND, login_in_progress_warning),
        ],
        allow_reentry=False,
    )
    ...
    # Forward Message to All Groups
    application.add_handler(CallbackQueryHandler(gm_forward_set_handler, pattern='^gm_forward_set$'))
    application.add_handler(CallbackQueryHandler(gm_forward_toggle_handler, pattern='^gm_forward_toggle$'))
    ...
    # Forward message capture — must be before text handler to catch forwarded messages
