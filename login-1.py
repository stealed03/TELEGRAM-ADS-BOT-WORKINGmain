...
class LoginHandler:
    ...
    async def _send_otp(self, user_id, phone):
        """Internal helper — connect client and request OTP. Returns client or raises."""
    ...
    async def receive_otp(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Receive OTP and login — handles expired OTP by auto-resending"""
        user_id = update.effective_user.id
        ...
        try:
            ...
            # Auto-resend OTP — user doesn't need to restart
            try:
                await self._send_otp(user_id, phone)
                await update.message.reply_text(
                ...
                return OTP  # stay on OTP state with fresh code
            ...
            await update.message.reply_text(
                f"⏳ **Flood Wait!**\n\n"
                f"Bahut zyada attempts. **{e.seconds} seconds** baad dobara try karo.",
                parse_mode='Markdown'
            )
            return ConversationHandler.END
            ...
            err = str(e)
            # Catch any remaining "expired" or "SignInRequest" edge cases
    ...
    async def receive_password(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if "password" in err or "invalid" in err:
                ...
                return PASSWORD  # retry password
            await update.message.reply_text(
                f"❌ 2FA error:\n`{str(e)}`\n\n/start se try again.",
                parse_mode='Markdown'
            )
