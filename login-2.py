...
# Device info — realistic phone device string, Telegram won't flag it
...
class LoginHandler:
    ...
    async def _cleanup_user(self, user_id):
        """Disconnect and remove any existing temp client for user."""
    ...
    async def _send_otp(self, user_id, phone):
        """
        Create ONE fresh client with realistic device info and request OTP.
        Always disconnects old client first to avoid multiple pending sessions.
        """
        api_id = self.temp_data[user_id]['api_id']
        api_hash = self.temp_data[user_id]['api_hash']

        # Disconnect old client if any — critical to avoid duplicate sessions
        old_client = self.temp_data[user_id].get('temp_client')
        if old_client:
            try:
                await old_client.disconnect()
                ...
                pass
            self.temp_data[user_id].pop('temp_client', None)
            self.temp_data[user_id].pop('phone_code_hash', None)

        # Realistic device info — avoids Telegram "Automation" flag
